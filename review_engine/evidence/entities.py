...
PATTERNS = {
    ...
    "company": re.compile(r"\b[A-Z][A-Za-z&' -]{1,50}\s(?:LLC|Inc\.?|Corp\.?|Corporation|Company|Co\.|Ltd\.?)\b"),
    "person": re.compile(r"\b(?:Mr\.|Ms\.|Mrs\.|Dr\.)?\s?[A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,2}\b"),
    "location": re.compile(r"\b(?:at|in|from)\s+([A-Z][A-Za-z .'-]+,\s*[A-Z]{2})\b"),
    "document_type": re.compile(r"\b(?:invoice|timecard|payroll|witness statement|investigation report|termination letter|policy|expense report|approval|contract)\b", re.I),
}
    ...
    for chunk in chunks:
        for entity_type, pattern in PATTERNS.items():
            for match in pattern.finditer(chunk.text):
                ...
                if key not in seen:
                    entities.append({"entity_type": entity_type, "value": value, "source_ref": chunk.source_ref})
                    seen.add(key)
        for sentence in re.split(r"(?<=[.!?])\s+", chunk.text):
            if PATTERNS["date"].search(sentence) and len(sentence) <= 500:
                key = ("event", sentence.lower(), chunk.source_ref)
                if key not in seen:
                    entities.append({"entity_type": "event", "value": sentence.strip(), "source_ref": chunk.source_ref})
                    seen.add(key)
    return entities
